...
class mcis_srv:
    def __init__(self, data_queue):
        self.db = sql_function.sql_func(config.database_name)  # Используется sql_function для работы с базой данных
    #     self.server_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    #     self.server_socket.bind((f'{config.server_host[0]}', config.server_host[1]))  # Привязываем сокет к IP и порту
    #     self.data_queue = data_queue  # Очередь для передачи данных в основной поток

    # def start_mcis_srv(self):
    #     """Запускает сервер для прослушивания входящих сообщений"""
    ...
    #     """Метод для отправки данных через сокет"""
    #     message = json.dumps(data)  # Преобразуем данные в строку
    #     self.server_socket.sendto(message.encode(), host)
    #     print(f"Message {data} sent to host {host}")
