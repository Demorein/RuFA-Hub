...
const EditPostPage: React.FC = () => {
  ...
  const { getPostById, updatePost } = useCodePosts();
  const { user, isAuthenticated } = useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    if (!isAuthenticated) {
      toast.error('Необходимо войти в систему для редактирования публикации');
      navigate('/login');
      return;
    }
    ...
    if (id) {
      ...
      if (post) {
        ...
        if (user?.id !== post.authorId) {
          toast.error('У вас нет прав на редактирование этой публикации');
          navigate('/posts/' + id);
          return;
        }
    ...
    setIsLoading(false);
  }, [id, getPostById, navigate, user, isAuthenticated]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    
    if (!title.trim() || !description.trim() || !code.trim() || !id) {
    ...
    try {
      ...
      toast.success('Публикация обновлена успешно!');
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold mb-6">Редактирование публикации</h1>
